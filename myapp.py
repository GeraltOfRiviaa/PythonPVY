...
def second():
    ...
    print(f"You would weight {physics.earth_to_moon_weight(weight)} kg")

    speed = 0
    speed = digit_input(speed,'How fast are you going rn in km/h?: ')
    print(f"You are going {physics.km_per_hour_to_speed_of_light(speed)} % of the speed of light")

    lower_speed = 0
    lower_speed = digit_input(lower_speed, "Puts some speed in km/h and see how many times you broke the sound barier: ")
    print(f"You broke the sound barier {physics.broke_sound_barier(lower_speed)}")  
#main()
second()